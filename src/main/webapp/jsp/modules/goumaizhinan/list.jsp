...
<body>
		...
		<div id="content">
				...
				<div class="container mt-0">
					<div class="row breadcrumb-bar">
						<div class="col-md-6">
							<h3 class="block-title">购买指南管理</h3>
						</div>
						<div class="col-md-6">
							<ol class="breadcrumb">
								...
								<li class="breadcrumb-item"><span>购买指南管理</span></li>
								<li class="breadcrumb-item active"><span>购买指南列表</span></li>
							</ol>
						</div>
					</div>
				</div>
				...
				<div class="row">
					...
					<div class="col-md-12">
						<div class="widget-area-2 lochana-box-shadow">
							<h3 class="widget-title">购买指南列表</h3>							
								...
								<table id="tableId" class="table table-bordered table-striped">
									<thead>
										<tr>
											...
											<th>图片</th>
											<th onclick="sort('jianshu')">简述<i id="jianshuIcon" class="fa fa-sort"></i></th>
											<th onclick="sort('wenzhangneirong')">文章内容<i id="wenzhangneirongIcon" class="fa fa-sort"></i></th>
	...
	<script>
		...
		function setDataRow(item,number){
			...
			var jianshuCell  = document.createElement('td');

						 jianshuCell.innerHTML = item.jianshu;


			row.appendChild(jianshuCell);
			var wenzhangneirongCell  = document.createElement('td');

						  //如果图文详情包含图片
			  if(item.wenzhangneirong && item.wenzhangneirong.indexOf('img') != -1){
				//暂时只考虑图片+文字 和 文字+图片的情况
				var beginIndex = item.wenzhangneirong.indexOf('<img');
				var endIndex = item.wenzhangneirong.indexOf('>');				
				wenzhangneirongCell.innerHTML = item.wenzhangneirong.substring(beginIndex, endIndex+1).replace("img","img width='100' height='100'");				
			  }else{
			  	 if(item.wenzhangneirong != null && item.wenzhangneirong != "" && item.wenzhangneirong.length >= 11){
			  		wenzhangneirongCell.innerHTML = item.wenzhangneirong.substring(0, 10) + "..."; 
			  	}else{
			  		wenzhangneirongCell.innerHTML = item.wenzhangneirong;
			 	}
			  }
				  


			row.appendChild(wenzhangneirongCell);
        ...
        function remove(id) { 
            ...
            if (mymessage == true) {
				...
				httpJson("goumaizhinan/delete","POST",paramArray,(res)=>{
					if(res.code == 0){
						getDataList();		
						alert('删除成功');
					}
		...
		function crossTable(id,crossTableName){
			...
			window.sessionStorage.setItem('crossTableName',"goumaizhinan");
