...
<body>
		...
		<div id="content">
				...
				<div class="container mt-0">
					<div class="row breadcrumb-bar">
						<div class="col-md-6">
							<h3 class="block-title">买卖中心管理</h3>
						</div>
						<div class="col-md-6">
							<ol class="breadcrumb">
								...
								<li class="breadcrumb-item active"><span>买卖中心列表</span></li>
							</ol>
						</div>
					</div>
				</div>
				...
				<div class="row">
					...
					<div class="col-md-12">
						<div class="widget-area-2 lochana-box-shadow">
							<h3 class="widget-title">买卖中心列表</h3>							
							<div class="table-responsive mb-3" id="tableDiv">
								<div class="col-sm-12">
									...
									<select name="jiagequjian" id="jiagequjianSelect" class="form-control form-control-sm" aria-controls="tableId">
										<option value=" "> </option>
									...
									<input type="text" id="zhuangbeiSearch" class="form-control form-control-sm" placeholder="" aria-controls="tableId">
									<label>
										道具
									</label>
									<input type="text" id="daojuSearch" class="form-control form-control-sm" placeholder="" aria-controls="tableId">
								...
								<table id="tableId" class="table table-bordered table-striped">
									<thead>
										<tr>
											...
											<th onclick="sort('maijiazhanghao')">卖家账号<i id="maijiazhanghaoIcon" class="fa fa-sort"></i></th>
											<th onclick="sort('maijiaxingming')">卖家姓名<i id="maijiaxingmingIcon" class="fa fa-sort"></i></th>
											<th onclick="sort('maijiashouji')">卖家手机<i id="maijiashoujiIcon" class="fa fa-sort"></i></th>
											<th onclick="sort('youxileixing')">游戏类型<i id="youxileixingIcon" class="fa fa-sort"></i></th>
											<th onclick="sort('jiagequjian')">价格区间<i id="jiagequjianIcon" class="fa fa-sort"></i></th>
											<th onclick="sort('jiage')">价格<i id="jiageIcon" class="fa fa-sort"></i></th>
											<th onclick="sort('youxiqufu')">游戏区服<i id="youxiqufuIcon" class="fa fa-sort"></i></th>
											<th onclick="sort('jiaosemingcheng')">角色名称<i id="jiaosemingchengIcon" class="fa fa-sort"></i></th>
											<th onclick="sort('dengji')">等级<i id="dengjiIcon" class="fa fa-sort"></i></th>
											<th onclick="sort('zhuangbei')">装备<i id="zhuangbeiIcon" class="fa fa-sort"></i></th>
											<th onclick="sort('daoju')">道具<i id="daojuIcon" class="fa fa-sort"></i></th>
											<th onclick="sort('zhanghaoxiangqing')">账号详情<i id="zhanghaoxiangqingIcon" class="fa fa-sort"></i></th>
	...
	<script>
		...
		function search(){
			...
			var jiagequjianIndex = document.getElementById("jiagequjianSelect").selectedIndex;
			var jiagequjianOptions = document.getElementById("jiagequjianSelect").options;
			var jiagequjianSelectedOption = jiagequjianOptions[jiagequjianIndex]
			if(jiagequjianSelectedOption.text != null && jiagequjianSelectedOption.text != ''){
				searchForm.jiagequjian = "%" + jiagequjianSelectedOption.text + "%";
			}
			if($('#zhuangbeiSearch').val() != null && $('#zhuangbeiSearch').val() != ''){
				searchForm.zhuangbei ="%" +  $('#zhuangbeiSearch').val() + "%";
			}
			
			if($('#daojuSearch').val() != null && $('#daojuSearch').val() != ''){
				searchForm.daoju ="%" +  $('#daojuSearch').val() + "%";
			}
        ...
        function getDataList() {
			http("maimaizhongxin/page","GET",{
				...
				zhuangbei : searchForm.zhuangbei,	
				daoju : searchForm.daoju,	
			},(res)=>{
				if(res.code == 0){
					clear();
		...
		function setDataRow(item,number){
			...
			var maijiashoujiCell  = document.createElement('td');

						 maijiashoujiCell.innerHTML = item.maijiashouji;


			row.appendChild(maijiashoujiCell);
			...
			var jiaosemingchengCell  = document.createElement('td');

						 jiaosemingchengCell.innerHTML = item.jiaosemingcheng;


			row.appendChild(jiaosemingchengCell);
			...
			var zhuangbeiCell  = document.createElement('td');

						 zhuangbeiCell.innerHTML = item.zhuangbei;


			row.appendChild(zhuangbeiCell);
			var daojuCell  = document.createElement('td');

						 daojuCell.innerHTML = item.daoju;


			row.appendChild(daojuCell);
            ...
            crossBtn1.innerHTML = "账号购买"
			crossBtn1.setAttribute("type","button");
			crossBtn1.setAttribute("class","btn btn-primary btn-sm");
			crossBtn1.setAttribute("onclick","crossTable(" + item.id + ",\"goumaidingdan\")");
			if(crossBtnControl('账号购买') == true){
        ...
        function remove(id) { 
            ...
            if (mymessage == true) {
				...
				httpJson("maimaizhongxin/delete","POST",paramArray,(res)=>{
					if(res.code == 0){
						getDataList();		
						alert('删除成功');
					}
		...
		function crossTable(id,crossTableName){
			...
			window.sessionStorage.setItem('crossTableName',"maimaizhongxin");
